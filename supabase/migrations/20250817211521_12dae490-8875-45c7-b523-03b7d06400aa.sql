...
-- Drop existing policies if they exist (policies don't support IF NOT EXISTS)
DROP POLICY IF EXISTS "Public can view photos" ON storage.objects;
DROP POLICY IF EXISTS "Admins and techs can upload photos" ON storage.objects;
DROP POLICY IF EXISTS "Admins and techs can update photos" ON storage.objects;
...
CREATE POLICY "Admins and techs can upload photos"
ON storage.objects
FOR INSERT
WITH CHECK (
  bucket_id = 'photos'
...
CREATE POLICY "Admins and techs can update photos"
ON storage.objects
FOR UPDATE
USING (
  bucket_id = 'photos'
