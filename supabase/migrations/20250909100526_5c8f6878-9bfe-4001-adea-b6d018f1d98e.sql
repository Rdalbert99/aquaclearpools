...
DO $$ BEGIN
  IF EXISTS (
    ...
    PERFORM 1 FROM pg_policies WHERE schemaname='public' AND tablename='invitation_security_summary';
    IF FOUND THEN
      -- remove all existing policies
      FOR policy_record IN SELECT polname FROM pg_policies WHERE schemaname='public' AND tablename='invitation_security_summary'
      LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.invitation_security_summary', policy_record.polname);
      END LOOP;
    END IF;

    -- Admin-only SELECT
    EXECUTE $$CREATE POLICY "Admins only can view invitation security summaries"
...
-- Helper: create a trigger safely
...
DO $$ BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='chemical_calculations' AND column_name='created_at') THEN
    -- some tables only have created_at; skip if no updated_at
    NULL;
  END IF;
