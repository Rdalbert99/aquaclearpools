-- CRITICAL SECURITY FIX: Fix Clients Table RLS Policies Role Assignment
...
-- The clients table contains sensitive data:
-- - Customer names and contact information  
-- - Email addresses and phone numbers
-- - Home/business addresses
-- - Service rates and billing information
-- - QuickBooks customer IDs
...
WITH CHECK (
  get_current_user_role() = 'tech' 
  AND assigned_technician_id = auth.uid()
  -- Prevent techs from changing critical business data
  AND OLD.customer = NEW.customer
  AND OLD.service_rate = NEW.service_rate
  AND OLD.qb_customer_id = NEW.qb_customer_id
);
...
BEGIN
  ...
  IF TG_OP = 'SELECT' THEN
    -- Note: This trigger won't fire on SELECT, but we document the intent
    RETURN NULL;
    ...
    PERFORM log_security_event_enhanced(
      ...
      jsonb_build_object(
        ...
        'contact_info_changed', (
          ...
          OLD.contact_address != NEW.contact_address
        ),
        'billing_info_changed', (OLD.service_rate != NEW.service_rate),
        'operation', 'UPDATE'
      ),
      CASE 
        WHEN OLD.service_rate != NEW.service_rate THEN 'warning'
        WHEN (OLD.contact_email != NEW.contact_email OR 
              OLD.contact_phone != NEW.contact_phone OR 
              OLD.contact_address != NEW.contact_address) THEN 'info'
        ELSE 'info'
      END
    );
    RETURN NEW;
  ...
  PERFORM log_security_event_enhanced(
    ...
    jsonb_build_object(
      ...
      'measures_applied', jsonb_build_array(
        'converted_policies_to_authenticated_role',
...
'MAXIMUM SECURITY: Client business data and contact information protected with authenticated-only RLS policies, comprehensive audit logging, and restricted admin access functions. All data access is monitored and logged.';